...
void read_dipsw(dsw_t* pdsw)
{
    ...
    // DSW1 
    // DSW2 
    // DSW3 : Win   Mac     change keymap

    // lo-active
    pdsw->dsw0_opt  = ((PIND & (1 << 4)) == 0)? DSW_OPT_ON    :DSW_OPT_OFF;    // DSW0 PD4: ON(lo)=ON, OFF(hi)=Win
    ...
    pdsw->dsw3_host = ((PINE & (1 << 6)) == 0)? DSW_KEYMAP_WIN:DSW_KEYMAP_MAC; // DSW3 PE6: ON(lo)=Win,  OFF(hi)=Mac
